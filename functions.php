...
function koyori_register_external_cover_meta() {
    foreach (array('post', 'page', 'shuoshuo') as $post_type) {
        if (post_type_exists($post_type)) {
            register_post_meta($post_type, '_koyori_external_cover_url', array(
                'type'              => 'string',
                'single'            => true,
                'default'           => '',
                'show_in_rest'      => true,
...
function theme_folder_check_on_admin_init() {
    ...
    // Koyori is an independent derivative theme; do not rename it back to upstream.
    $correct_theme_folder = 'koyori';
    $user_locale = get_user_locale();
...
// Koyori customizations; see koyori-custom.php and UPSTREAM.md.
require_once get_template_directory() . '/koyori-custom.php';
